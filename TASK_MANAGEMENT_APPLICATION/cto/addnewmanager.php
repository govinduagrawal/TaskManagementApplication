...
// ini_set('display_errors', 'On');
 ...
 session_start(); 
if(!isset($_SESSION['cto_id'])) // to check user is admn or not
{
  session_destroy();
...
<body>
  ...
  <div class="w5layouts-main"> 
    <div class="updateprofile-layer">
      <h1 style="color:white;">Add New Manager</h1>
  ...
  if(isset($_POST["createuser"])) // on submit extact above orm details
  {
   ...
   $result = $function->insert('admin',array('admin_fname','admin_lname','admin_email','admin_password','admin_image','admin_status','admin_gender'),array("'$adminFname'", "'$adminLname'","'$adminEmail'","'$adminPassword'","'$uploadfile'","'$adminStatus'","'$adminGender'"));
   // $result = $function->insert('user',array('user_fname','user_lname','user_email','user_password','user_image','user_status','user_gender'),array("'"$_POST['fname']"'", "'"$_POST['lname']"'","'"$_POST['email']"'","'"$_POST['password']"'","'$imagePath'","'"$_POST['userstatus']"'","'"$_POST['gender']"'"));
    ...
    {?>
      ...
      <div id="popup"><?php echo $adminFname." ".$adminLname." "; ?>SUCCESSFULLY ADDED</div>
